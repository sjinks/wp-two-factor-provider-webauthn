...
version_marker_line_re='^[+-].*(Version:|TFA_WEBAUTHN_VERSION)'
version_marker_escaped_re='\\n[+-].*(Version:|TFA_WEBAUTHN_VERSION)'
...
if printf '%s' "$payload" | grep -Eiq "$index_path_re" \
  && ( printf '%s' "$payload" | grep -Eiq "$version_marker_line_re" \
    || printf '%s' "$payload" | grep -Eiq "$version_marker_escaped_re" ) \
