...
<div id="webauthn-security-keys-section">
	<h3><?php esc_html_e( 'Security Keys (WebAuthn)', '2fa-wa' ); ?></h3>

	<noscript>
		<div class="notice inline notice-error">
			<p><?php esc_html_e( 'You need to enable JavaScript to manage security keys.', '2fa-wa' ); ?></p>
		</div>
	</noscript>
...
<?php if ( ! is_ssl() ) : ?>
	<div class="notice inline notice-error">
		<p>
			<?php esc_html_e( 'WebAuthn requires an HTTPS connection. You will be unable to add new security keys over HTTP.', '2fa-wa' ); ?>
		</p>
	</div>
<?php else : ?>
	...
	<div class="hide-if-no-js add-webauthn-key">
		<p>
			<label for="webauthn-key-name" style="vertical-align: middle"><strong><?php esc_html_e( 'Key name:', '2fa-wa' ); ?></strong></label>
			<input type="text" required="required" id="webauthn-key-name" value="" style="vertical-align: middle" />
			<button type="button" class="button button-secondary" style="vertical-align: middle"><?php echo esc_html( _x( 'Register New Key', 'security key', 'two-factor' ) ); ?></button>
		</p>
...
<script type="text/x-template" id="webauthn-no-keys">
	<tr class="no-items"><td class="colspanchange" colspan="<?php echo esc_attr( (string) $table->get_column_count() ); ?>"><?php $table->no_items(); ?></td></tr>
</script>
<script type="text/x-template" id="webauthn-revoke-confirm">
	<div class="confirm-revoke" role="alert">
		<p><?php esc_html_e( 'Are you sure to revoke this key?', '2fa-wa' ); ?></p>
...
<script type="text/x-template" id="webauthn-rename-key">
	<div class="rename-key">
		<p>
			<label>
				<?php esc_html_e( 'New name:', '2fa-wa' ); ?><br/>
